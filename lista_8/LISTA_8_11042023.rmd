...
# e) Calcule o risco relativo de um modelo sem efeito aleatório espacial
...
O modelo BYM2 (Besag, York e Mollie, segunda versão) e a SMR (Standardized Mortality Ratio) são duas abordagens diferentes para entender a relação espacial de um determinado evento. O modelo BYM2 é um modelo bayesiano que considera tanto a informação espacial quanto a informação sobre a covariável (por exemplo, quantidade de domicílios) para estimar as taxas de mortalidade padronizadas. Ele modela a variável resposta em cada área como uma combinação linear das covariáveis e dos efeitos espaciais não observados. Além disso, também inclui uma estrutura de autocorrelação espacial para lidar com a dependência espacial dos dados. Ao permitir que as covariáveis sejam incluídas na análise, pode aumentar a precisão das estimativas das taxas de mortalidade padronizadas. Uma outra questão é que o modelo BYM2 é capaz de lidar com dados ausentes e desigualdades na população em risco, o que é uma vantagem sobre outros modelos que não levam esses fatores em consideração.
Por outro lado, a SMR (Standardized Mortality Ratio) é uma medida de risco que compara a taxa de mortalidade observada em uma determinada área com a taxa de mortalidade esperada com base em uma população de referência. Ela é calculada dividindo a taxa de mortalidade observada pela taxa de mortalidade esperada e multiplicando por 100. Uma SMR maior que 100 indica que a taxa de mortalidade observada é maior do que a taxa de mortalidade esperada. Ela é uma medida simples e fácil de calcular, mas tem algumas limitações. Uma das principais limitações da SMR é que ela pressupõe que a taxa de mortalidade esperada é constante em todas as áreas e em todos os anos, o que nem sempre é verdade. Além disso, a SMR não considera outras covariáveis que podem influenciar a mortalidade, como a idade e o sexo da população.
Enquanto a SMR é uma medida descritiva útil para comparar as taxas de mortalidade entre diferentes áreas, ela não leva em conta a informação espacial ou outras covariáveis que podem influenciar a mortalidade. Por outro lado, o modelo BYM2 fornece uma abordagem mais sofisticada e mais robusta estatisticamente para estimar as taxas de mortalidade padronizadas, levando em conta a informação espacial e outras covariáveis.

No caso específico dos resultados obtidos na letras anteriores, as taxas do modelo bayesiano e SMR são semelhantes, principalmente devido ao fato de que o efeito da covariável domicílio é fraco no modelo utilizado.